...
const neonSchema = process.env.NEON_SCHEMA;
if (!neonSchema || !["dev", "prod"].includes(neonSchema)) {
...
await pool.query(`SET search_path TO ${neonSchema}`);

const db = drizzle(pool, { schema });

// Also set search_path for the drizzle connection
await db.execute(sql.raw(`SET search_path TO ${neonSchema}`));

console.log(`Seeding Neon [${neonSchema}] with test data...`);

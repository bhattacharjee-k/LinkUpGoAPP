...
export async function registerRoutes(
  ...
  // Proposed times routes
  app.get("/api/sessions/:sessionId/proposed-times", async (req, res) => {
    try {
      // @ts-ignore
      const userId = req.session?.userId;
      if (!userId) {
      ...
      const times = await storage.getSessionProposedTimes(req.params.sessionId);
      res.json(times);
    } catch (error: any) {
      res.status(400).json({ message: error.message });
    }
  ...
  app.post("/api/sessions/:sessionId/proposed-times", async (req, res) => {
    try {
      // @ts-ignore
      const userId = req.session?.userId;
      if (!userId) {
      ...
      const { proposedDate, timeStart, timeEnd, note } = req.body;
      
      if (!proposedDate || !timeStart || !timeEnd) {
        return res.status(400).json({ message: "Date and time range are required" });
      }
      
      const created = await storage.createProposedTime({
  ...
  app.post("/api/proposed-times/:id/vote", async (req, res) => {
    try {
      // @ts-ignore
      const userId = req.session?.userId;
      if (!userId) {
      ...
      const updated = await storage.voteForProposedTime(req.params.id, userId);
      if (!updated) {
        return res.status(404).json({ message: "Proposed time not found" });
      }
      res.json(updated);
    } catch (error: any) {
      res.status(400).json({ message: error.message });
  ...
  app.delete("/api/proposed-times/:id", async (req, res) => {
    try {
      // @ts-ignore
      const userId = req.session?.userId;
      if (!userId) {
